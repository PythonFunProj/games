...
image = pygame.image.load(r'carré.jpg')
image = pygame. transform. scale(image, (lensquare, lensquare))
image2 = pygame.image.load(r'drapeau.jpg')
image2 = pygame. transform. scale(image2, (lensquare, lensquare))
image3 = pygame.image.load(r'mine.jpg')
image3 = pygame. transform. scale(image3, (lensquare, lensquare))
image4 = pygame.image.load(r'mine2.jpg')
couleur = [(0, 0, 175), (0, 200, 0), (175, 0, 0), (0, 0, 75), (0, 0, 75), (0, 0, 75)]
def board1():
    ...
    for i in range(0, length):
        for j in range(0, width):
            ...
            screen.blit(font.render(str(nombre), True, couleur[boardvisible[i, j]-1]), (spacex + lensquare*i+10, spacey + lensquare*j+4))
    ...
    if pygame.mouse.get_pressed()[0] and click == 0:
        ...
        if board[x, y] == 9:
            for i in range(0, length):
                for j in range(0, width):
                    if board[i, j] == 9:
                        display_surface.blit(image3, (spacex + lensquare * i, spacey + lensquare * j))
            while done:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        done = False
